...
def quad_10_10_func(x):
    ...
    mat = scipy.io.loadmat(r"quad_10_10_Q.mat")
    Q = mat["Q"]
...
def rosenbrock_2_grad(x):
    return np.array(
        [2 * (x[0] - 1) - 400 * x[0] * (x[1] - x[0] ** 2), 200 * (x[1] - x[0] ** 2)]
    )
...
def rosenbrock_100_func(x):
    return sum(
        (1 - x[i]) ** 2 + 100 * (x[i + 1] - x[i] ** 2) ** 2 for i in range(len(x) - 1)
    )
...
def exp_10_func(x):
    x0 = np.zeros(10)
    x0[0] = 1
    return (np.exp(x[0]) - 1) / (np.exp(x[0]) + 1) + 0.1 * np.exp(-x[0]) + np.sum(
        (x[1:] - 1) ** 4
    ), x0
...
def exp_10_Hess(x):
    H = np.zeros((10, 10))
    H[0, 0] = (
        2 * np.exp(x[0]) * (np.exp(x[0]) + 1) ** (-2)
        - 4 * (np.exp(x[0]) - 1) * np.exp(x[0]) * (np.exp(x[0]) + 1) ** (-3)
        - 0.1 * np.exp(-x[0])
    )
